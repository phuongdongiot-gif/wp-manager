    ...
    <div className="min-h-screen bg-white dark:bg-[#050505] flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-[280px] bg-[#FAFAFA] dark:bg-[#0A0A0A] border-r border-gray-200 dark:border-white/5 flex flex-col h-auto md:h-screen sticky top-0">
        <div className="p-8 border-b border-gray-200 dark:border-white/5 flex items-center justify-center">
          <h1 className="text-2xl font-serif font-bold tracking-[0.2em] uppercase text-gray-900 dark:text-white flex items-center">
            WP<span className="text-primary ml-2 font-light">Manager</span>
          </h1>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center w-full px-6 py-4 text-xs font-medium uppercase tracking-[0.15em] transition-all duration-500 border-l-[3px] ${
                  isActive
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-3 relative">
          <div className="text-[10px] font-medium text-gray-400 uppercase tracking-[0.2em] mb-6 px-1">
            Website Của Bạn
          </div>
          ...
          {sites.map(site => (
            ...
            >
              <div className="flex flex-col truncate pr-2">
                <span className="font-semibold text-sm truncate tracking-wide">{site.siteName && site.siteName !== site.username && site.siteName !== 'New Site' ? site.siteName : site.url.replace(/^https?:\/\//, '').replace(/\/$/, '')}</span>
                <span className="text-[10px] uppercase tracking-widest opacity-60 truncate mt-1 flex gap-1.5">
          ...
          <Link 
            to="/login"
            className="w-full mt-8 flex items-center justify-center p-4 rounded-none border border-dashed border-gray-300 dark:border-white/20 text-xs font-semibold uppercase tracking-[0.1em] text-gray-500 dark:text-gray-400 hover:text-primary hover:border-primary hover:bg-primary/5 dark:hover:text-primary transition-all duration-500"
          >
        ...
        <div className="p-6 border-t border-gray-200 dark:border-white/5 bg-[#F5F5F5] dark:bg-[#080808]">
          <button className="flex items-center text-xs font-semibold uppercase tracking-[0.1em] text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors duration-500 w-full p-3 rounded-none hover:bg-gray-200 dark:hover:bg-white/5">
