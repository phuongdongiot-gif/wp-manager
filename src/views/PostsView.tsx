    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex justify-between items-end mb-12 pb-6 border-b border-gray-200 dark:border-white/10">
        <div>
          ...
          <p className="text-xs tracking-[0.1em] uppercase text-gray-500 dark:text-gray-400 mt-3 inline-block">Nội dung tin tức và kiến thức</p>
        </div>
        {!showForm && (
          <button 
            onClick={() => {
      ...
      {showForm && (
        <div className="bg-white dark:bg-[#0A0A0A] rounded-none shadow-2xl border border-gray-200 dark:border-white/5 p-12 mb-12 transition-all relative">
          ...
          <div className="flex justify-between items-center mb-10 pb-6 border-b border-gray-100 dark:border-white/5">
            <h3 className="text-2xl font-serif text-gray-900 dark:text-white tracking-[0.1em] uppercase">{editingPostId ? 'Chỉnh sửa Bài viết' : 'Viết Bài Mới'}</h3>
            <button type="button" onClick={() => {
              ...
              setFeaturedImagePreview('');
              setShowForm(false);
            }} className="text-[10px] uppercase tracking-[0.15em] font-medium text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors underline underline-offset-4 decoration-transparent hover:decoration-gray-500">Huỷ bỏ</button>
          </div>
          <form onSubmit={handleCreateNew} className="space-y-12">
            <div className="relative group">
              <input 
                ...
                placeholder=" " 
              />
              <label className="absolute left-0 top-6 text-gray-400 dark:text-gray-500 text-sm peer-focus:text-xs peer-focus:-translate-y-7 peer-focus:text-primary peer-valid:text-xs peer-valid:-translate-y-7 transition-all duration-300 pointer-events-none uppercase tracking-[0.1em]">Tiêu đề Bài viết</label>
              <div className="absolute bottom-0 left-0 w-0 h-[1px] bg-primary transition-all duration-500 group-focus-within:w-full"></div>
            </div>
            <div>
              <label className="block text-[10px] font-medium uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400 mb-6">Nội dung Bài viết</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-gray-100 dark:border-white/10">
              <div className="bg-[#FAF9F6] dark:bg-transparent p-8 rounded-none border border-gray-200 dark:border-white/10 hover:border-primary/50 transition-colors">
                <label className="block text-sm font-serif uppercase tracking-[0.1em] text-gray-900 dark:text-white mb-6">Cấu hình SEO</label>
                <div className="space-y-6">
                  <div className="relative group">
                    ...
                    <label className="absolute left-0 top-4 text-gray-400 dark:text-gray-500 text-[10px] peer-focus:-translate-y-5 peer-focus:text-primary peer-valid:-translate-y-5 transition-all duration-300 pointer-events-none uppercase tracking-[0.15em] font-medium">Từ khoá chính (Focus Keyword)</label>
                  </div>
                  <div className="relative group">
                    ...
                    <label className="absolute left-0 top-4 text-gray-400 dark:text-gray-500 text-[10px] peer-focus:-translate-y-5 peer-focus:text-primary peer-valid:-translate-y-5 transition-all duration-300 pointer-events-none uppercase tracking-[0.15em] font-medium">Tiêu đề hiển thị SEO</label>
                  </div>
                  <div className="relative group pt-2">
                    <label className="block text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-[0.15em] font-medium mb-2">Thẻ mô tả (Meta Description)</label>
              ...
              <div className="bg-[#FAF9F6] dark:bg-transparent p-8 rounded-none border border-gray-200 dark:border-white/10 hover:border-primary/50 transition-colors">
                <label className="block text-sm font-serif uppercase tracking-[0.1em] text-gray-900 dark:text-white mb-6">Ảnh đại diện</label>
                <div className="flex flex-col items-start gap-4">
                  <input 
                    ...
                    className="text-[10px] text-gray-500 uppercase tracking-[0.1em] font-medium file:cursor-pointer file:mr-4 file:py-3 file:px-6 file:rounded-none file:border-0 file:border-r file:border-gray-200 dark:file:border-white/10 file:text-[10px] file:font-bold file:bg-white dark:file:bg-[#050505] file:text-primary hover:file:bg-primary/5 file:transition-colors"
                  />
            ...
            <div className="pt-8 border-t border-gray-100 dark:border-white/10">
              <label className="block text-[10px] font-medium uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400 mb-6">Danh mục Phân loại</label>
              {availableCategories.length > 0 ? (
                <div className="flex flex-wrap gap-3">
                  {availableCategories.map(cat => (
                    <label key={cat.id} className={`flex items-center px-4 py-2 rounded-none border cursor-pointer transition-colors duration-300 text-[10px] uppercase tracking-[0.1em] font-medium ${
                ...
                <div className="text-[10px] text-gray-400 uppercase tracking-[0.1em]">Danh mục đang trống.</div>
              )}
            </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-32 bg-transparent border border-gray-200 dark:border-white/10 rounded-none">
          ...
          <p className="text-gray-500 text-sm mb-8 tracking-wide font-light">Không tìm thấy bản ghi bài viết nào. Vui lòng thiết lập để tiếp tục.</p>
          <button onClick={() => setShowForm(true)} className="px-8 py-4 border border-primary text-primary hover:bg-primary hover:text-black dark:hover:text-black transition-colors font-semibold uppercase tracking-[0.2em] text-[10px]">Khởi tạo Dữ liệu</button>
        </div>
      ) : (
